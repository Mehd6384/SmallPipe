...
class World():
	...
	def get_state_elements(self, elements): 

		state = []
		for e in elements: 
			if e == 'joints_pos': 
				for p in self.robot.points: 
					for pp in p: 
						state.append(pp)
			if e == 'joints_angles': 

				for a in self.robot.angles: 
					state.append(np.pi*(a%360)/180)

			if e == 'target_pos':

				for p in self.target.position: 
					state.append(p)
			...
			if e == 'target_effector_vec': 

				v = self.get_vector_target_effector()
				for vc in v: 
					state.append(vc)
	...
	def render(self, draw_path = False, draw_target = False): 
		...
		self.draw(self.screen, self.size, draw_path = draw_path, draw_target = draw_target)

		pg.display.flip() 

	def draw(self,screen, screenSize , draw_path = False, draw_target = False): 
	...
	def observe_bis(self, names): 
		...
		states = []
		for n in names: 
			states.append(self.get_state_elements([n]))
	...
	def observe_image(self, dim = [64,64], invert = False, bw = True):

		pg.image.save(self.screen, 'state.png')
		state = Image.open('state.png')
		state = state.resize(dim, resample = Image.LANCZOS)
		state.save('stateL.png')

		np_state = np.array(state)
		np_state = np.transpose(np_state, [2,0,1])

		np_state = np_state.astype(float)
		np_state /= 255.

		np_state[np_state > 0.1] = 1.

		if invert: 
			np_state = 1.- np_state

		# if invert: 
		# 	np_state = 1.- (np_state/255.)
		# else: 
		# 	np.state = np_state/255.

		if bw: 
			np_state = np.average(np_state, axis = 0)
			np_state = np_state.reshape(1,64,64)
			# input(np_state.shape)
		...
		return np_state, complete	


	def observe_image_and_state(self, dim = [64,64], invert = False, bw = True): 
		...
		state_ll = []
		for o in self.obstacles: 
			state_ll.append(o.position[0])
			state_ll.append(o.position[1])

		for l in self.robot.points: 
			state_ll.append(l[0])
			state_ll.append(l[1])

		state_ll.append(self.target.position[0])
		state_ll.append(self.target.position[1])
		...
		return np_state, state_ll, complete	
	...
	def check_success(self): 
		...
		vec = self.get_vector_target_effector()
		d = magnitude(vec)
	...
	def step_deltas(self, delta, use_CNN = False): 
	...
	def step_astar(self, only_deltas = False): 
	...
	def step_astar_cnn(self, dim): 

		# how to draw without losing too much time ????????????????????????
		# i can't think of any clear and wise way 
		...
		data = self.step_real_vector_and_return_deltas(vector)
		return [data[0], self.observe_image(dim)]

	def step_control(self, deltas): 

		self.robot.rotate_ik(deltas)
		self.robot.compute_positions()
		self.steps += 1
	...
	@property
	def sizes(self): 
		return self.get_env_infos()

	def reset(self, for_CNN = True): 
...
# 			target_limits = [0.2,0.8,0.2,0.8], obstacles = 0, randomize_obstacles = True,
...
# 	while not done: 
# 		# if i %2 == 0: 
# 		# 	env.render(draw_path = False, draw_target = False)
# 		# else: 
# 		env.render(draw_path = False, draw_target = True)
# 		s1,s2,done = env.observe_image_and_state()
# 		deltas = env.step_astar(only_deltas = True)

# 		computed_state = env.get_state_elements(['joints_pos','target_pos'])
# 		angles = computed_state[0:6]
# 		pos = computed_state[3:]
